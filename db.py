...
current_app.config["SQLALCHEMY_DATABASE_URI"] = f"mysql://{DB_CONNECT['username']}:{DB_CONNECT['password']}@{DB_CONNECT['server']}:3306/{DB_CONNECT['dbname']}?charset=utf8mb4"
...
class TodoList(db.Model):
      id = db.Column(db.Integer, primary_key=True)
      content = db.Column(db.String(30), default="To do something")
...
def createTutorial(user_id):
      ...
      delTask = TodoList(content="할 일 삭제하기🗑", user_id=user_id)
      addCategory = TodoList(content="카테고리 추가하기➕", user_id=user_id)
      changeColor = TodoList(content="카테고리 테마색 바꾸기🎨", user_id=user_id)
      editCategoryName = TodoList(content="카테고리 이름 클릭해서 수정하기📝", user_id=user_id)
      delCategory = TodoList(content="카테고리 삭제하기🗑", user_id=user_id)
      ...
      tutorial.todos.append(delTask)
      tutorial.todos.append(addCategory)
      tutorial.todos.append(changeColor)
      tutorial.todos.append(editCategoryName)
      tutorial.todos.append(delCategory)
